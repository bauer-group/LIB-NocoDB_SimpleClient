...
@pytest.mark.skipif(SKIP_INTEGRATION, reason="Integration tests skipped (set SKIP_INTEGRATION=0 to run)")
class TestIntegration:
    ...
    def test_query_operations(self, integration_table):
        ...
        try:
            filtered_records = integration_table.get_records(
                where="(Name,isnotblank)",
                limit=3
            )

...
def validate_where_clause(where: str) -> str:
    ...
    for pattern in dangerous_patterns:
        if re.search(pattern, where.lower()):
            raise ValidationException("Potentially dangerous pattern detected in WHERE clause", field_name="where")
...
def validate_sort_clause(sort: str) -> str:
    ...
    for field in sort_fields:
        ...
        if not re.match(r'^[a-zA-Z_][a-zA-Z0-9_]*$', field_name):
            raise ValidationException(
                f"Invalid field name in SORT clause: {field_name}",
                field_name="sort"
            )
...
def validate_file_path(file_path: Union[str, Path]) -> Path:
    ...
    if isinstance(file_path, str):
        ...
        raise ValidationException("File path must be a string or Path object", field_name="file_path")
...
def validate_api_token(token: str) -> str:
    ...
    if len(token) < 10:
        raise ValidationException("API token must be at least 10 characters", field_name="api_token")

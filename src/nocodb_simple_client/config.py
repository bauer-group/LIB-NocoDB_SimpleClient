...
class NocoDBConfig:
    ...
    def from_env(cls, env_prefix: str = "NOCODB_") -> "NocoDBConfig":
        ...
        if not api_token:
            raise ValueError(f"Environment variable {env_prefix}API_TOKEN or {env_prefix}TOKEN is required")
        
        return cls(
            base_url=base_url,
            api_token=api_token,
            access_protection_auth=os.getenv(f"{env_prefix}PROTECTION_AUTH"),
            access_protection_header=os.getenv(f"{env_prefix}PROTECTION_HEADER", "X-BAUERGROUP-Auth"),

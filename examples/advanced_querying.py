...
from nocodb_simple_client import NocoDBClient, NocoDBTable, NocoDBException
...
def demonstrate_filtering(table: NocoDBTable):
    ...
    records = table.get_records(
        where="(Age,gt,25)",  # Greater than 25
        fields=["Id", "Name", "Age"],
        limit=5
    )
    ...
    records = table.get_records(
        where=f"(CreatedAt,gt,{yesterday})",
    ...
    print("\n7. NULL/Empty checks:")
    records = table.get_records(
        where="(Email,isblank)",  # Email is empty
    ...
    print("\n8. NOT NULL checks:")
    records = table.get_records(
        where="(Email,isnotblank)",  # Email is not empty
        fields=["Id", "Name", "Email"],
        limit=3
    )
...
def demonstrate_sorting(table: NocoDBTable):
    ...
    records = table.get_records(
        sort="-Id",  # Minus sign for descending
        fields=["Id", "Name"],
        limit=5
    )
    for record in records:
...
def demonstrate_field_selection(table: NocoDBTable):
    ...
    for record in records:
        print(f"   - ID: {record.get('Id')}, Name: {record.get('Name')}, Email: {record.get('Email')}")
